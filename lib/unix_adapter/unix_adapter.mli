(** Unix scheduler adapter: serialises blocking [Unix.file_descr] byte reads and validated resize requests through
...
(** Thread-safe: may be called concurrently with a {!run} loop reading on another thread. This direction and the
...
val read_step : t -> Unix.file_descr -> bytes -> (read_result, error) Err.t
(** One blocking {!Unix.read} on the descriptor into [buffer], ingested through the session. [buffer] is owned and
...
val run :
  ...
  unit
(** Blocking read loop: repeatedly calls {!read_step} until it reports {!Eof} (calling [on_outcome] for every ingested
    outcome along the way, including the final one) or reports an error (reported once via [on_error]; the loop then
    stops without calling [Session.finish]). *)
