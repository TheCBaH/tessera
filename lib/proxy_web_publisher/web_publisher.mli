(** Per-client web-rendering fan-out, the observe-only web publisher: turns each {!Tessera.outcome} the proxy relay
    already produces into at most one {!Tessera_web_rendering.Web_frame.t} Reset and one Delta per outcome, projected
    and JSON-encoded at most once per target ({!Tessera_web_rendering.Web_json}'s HTML/Canvas envelopes, mirroring
    {!Web_bridge.render} exactly), and fans that out to every attached client's own bounded pending queue.
...
val prepend_pending : t -> client -> string -> unit
(** Inserts one already-encoded control message immediately before the client's currently pending output. The web
    transport uses this only immediately after {!attach}, to put authoritative input state ahead of its Reset. *)
...
val note_outcome : t -> ?before:(target -> string) -> Tessera.outcome -> (unit, error) Err.t
(** Builds at most one neutral Reset {!Tessera_web_rendering.Web_frame.t} and one Delta per outcome, and encodes each at
    ...
    [max_pending_bytes], in which case clear its queue, enqueue [before] (when supplied) and that target's Reset message
    for *this* outcome instead (never resume with a delta after a drop), and leave [needs_reset = false] (the reset just
    sent resolves it). A [before]+Reset pair is never itself truncated or rejected for being large -- the bound is on
    backlog, not on one authoritative payload, matching {!Tessera_proxy_linux.Observer_server}'s equivalent snapshot
    behaviour.
